...
import {
	BundleSearchQuerySchema,
	type BundleSearchQuery,
...
function parsePackageName(
	name: string,
): { scope: string; packageName: string } | null {
 ...
 */
function getProvenanceSummary(version: {
 ...
 */
function getProvenanceFull(version: {
	publishMethod: string | null;
	provenance: unknown;
}) {
	if (version.publishMethod !== "oidc" || !version.provenance) {
...
export const bundleRoutes: FastifyPluginAsync = async (fastify) => {
		...
		handler: async (request) => {
			...
			const bundles = await Promise.all(
				packages.map(async (pkg) => {
					...
					const manifest = (latestVersion?.manifest ?? {}) as Record<
						string,
						unknown
					>;
					...
					return {
						...
						provenance: latestVersion
							? getProvenanceSummary(latestVersion)
							: null,
	...
	fastify.get("/@:scope/:package/index.json", {
		schema: {
			tags: ["bundles"],
			description:
				"Get multi-platform distribution index for a bundle (MCPB Index spec)",
			params: {
				type: "object",
				properties: {
					scope: { type: "string" },
		...
		handler: async (request, reply) => {
			...
			const latestVersion = await packageRepo.findVersionWithArtifacts(
				pkg.id,
				pkg.latestVersion,
			);
			if (!latestVersion) {
			...
			const bundleArtifacts = await Promise.all(
				latestVersion.artifacts.map(async (artifact) => {
					const url = await fastify.storage.getSignedDownloadUrlFromPath(
			...
			const index = {
				...
				annotations: {
					...(latestVersion.releaseUrl && {
						"dev.mpak.release.url": latestVersion.releaseUrl,
					}),
					...(latestVersion.provenanceRepository && {
						"dev.mpak.provenance.repository":
							latestVersion.provenanceRepository,
					}),
					...
					...(latestVersion.publishMethod && {
						"dev.mpak.provenance.provider":
							latestVersion.publishMethod === "oidc"
								? "github_oidc"
								: latestVersion.publishMethod,
					}),
				},
			...
			reply.header(
				"Content-Type",
				"application/vnd.mcp.bundle.index.v0.1+json",
			);
			return index;
		},
	});
	...
	fastify.get("/@:scope/:package/versions/:version", {
		...
		handler: async (request) => {
			...
			const packageVersion = await packageRepo.findVersionWithArtifacts(
				pkg.id,
				version,
			);
			...
			const artifacts = await Promise.all(
				packageVersion.artifacts.map(async (a) => {
					const downloadUrl =
						await fastify.storage.getSignedDownloadUrlFromPath(a.storagePath);
	...
	fastify.get("/@:scope/:package/versions/:version/download", {
		...
		handler: async (request, reply) => {
			...
			const { os: queryOs, arch: queryArch } = request.query as {
				os?: string;
				arch?: string;
			};
			const name = `@${scope}/${packageName}`;
			...
			const version =
				versionParam === "latest" ? pkg.latestVersion : versionParam;
			...
			if (wantsJson) {
				...
				expiresAt.setSeconds(
					expiresAt.getSeconds() +
						(config.storage.cloudfront.urlExpirationSeconds || 900),
				);
	...
	fastify.post("/announce", {
		...
		handler: async (request, reply) => {
			try {
				...
				const serverObj = manifest["server"] as
					| Record<string, unknown>
					| undefined;
				const serverType =
					(serverObj?.["type"] as string) ??
					(manifest["server_type"] as string);
				if (!serverType) {
					throw new BadRequestError(
				...
				if (serverJsonAsset) {
					try {
						fastify.log.info(
							`Fetching server.json from release ${release_tag}`,
						);
						const sjResponse = await fetch(
							serverJsonAsset.browser_download_url,
						);
						if (sjResponse.ok) {
							const sjData = (await sjResponse.json()) as Record<
								string,
								unknown
							>;
				...
				try {
					const txResult = await runInTransaction(async (tx) => {
							...
							await packageRepo.upsertPackage(
								{
									name,
									displayName:
										(manifest["display_name"] as string) ?? undefined,
						...
						const existingVersion = await packageRepo.findVersion(
							packageId,
							version,
							tx,
						);
							...
							try {
								...
								if (readmeResponse.ok) {
									...
									if (readmeData.content && readmeData.encoding === "base64") {
										...
										fastify.log.info(
											`Fetched README.md (${readme.length} chars)`,
										);
									}
								}
							} catch (readmeError) {
						...
						// Upsert version
						const { version: packageVersion, created } =
							await packageRepo.upsertVersion(
								packageId,
								{
									packageId,
									version,
						...
						totalArtifacts = await packageRepo.countVersionArtifacts(
							packageVersion.id,
							tx,
						);
					...
					try {
						...
						fastify.log.info(
							`Cleaned up after transaction failure: ${storagePath}`,
						);
					} catch (cleanupError) {
						fastify.log.error(
				...
				if (config.scanner.enabled && versionId) {
					triggerSecurityScan(fastify.prisma, {
						...
						fastify.log.error({ err }, "Failed to trigger security scan"),
					);
				}
