...
import {
  ...
  parseGitHubRepo,
  verifyPackageClaim,
} from '../services/github-verifier.js';
...
export const packageRoutes: FastifyPluginAsync = async (fastify) => {
    ...
    handler: async (request, reply) => {
      try {
        ...
        if (existingPackage) {
          ...
          if (existingPackage.claimedBy && existingPackage.claimedBy !== user.userId) {
            throw new ForbiddenError(
              'This package has been claimed by another user. You cannot publish to it.',
            );
          }
          ...
          if (existingVersion) {
            throw new ConflictError(
              `Version ${version} already exists. Cannot overwrite existing versions.`,
            );
          }
        }
        ...
        if (githubRepo && user.githubUsername) {
          try {
            ...
            if (verification.verified) {
              ...
              wasAutoClaimed = true;
              fastify.log.info(
                `Auto-claimed package ${packageName} for user ${user.githubUsername}`,
              );
            } else {
        ...
        const {
          ...
          sha256,
          size,
        } = await fastify.storage.saveBundle(scope, parsedPackageName, version, buffer);
        ...
        if (result.githubRepo) {
          fetchGitHubRepoStats(result.githubRepo)
            .then((stats) => {
              if (stats) {
                packageRepo
                  .updateGitHubStats(result.packageId, stats)
                  .catch((err: unknown) =>
                    fastify.log.error({ err }, 'Failed to update GitHub stats'),
                  );
              }
            })
            .catch((err: unknown) => fastify.log.error({ err }, 'Failed to fetch GitHub stats'));
        }
        ...
        const downloadUrl = fastify.storage.getBundleUrl(scope, parsedPackageName, version);
        ...
        if (result.wasAutoClaimed) {
          ...
          response.message =
            'Package published and automatically claimed based on mpak.json verification';
        }
  ...
  fastify.get('/@:scope/:package', {
    ...
    handler: async (request) => {
      ...
      if (pkg.githubRepo) {
        ...
        if (isStale) {
          fetchGitHubRepoStats(pkg.githubRepo)
            .then((stats) => {
              if (stats) {
                packageRepo
                  .updateGitHubStats(pkg.id, stats)
                  .catch((err: unknown) =>
                    fastify.log.error({ err }, 'Failed to update GitHub stats'),
                  );
              }
      ...
      const versionsWithArtifactsAndScans = await packageRepo.getVersionsWithArtifactsAndScans(
        pkg.id,
      );

      // Get latest version manifest
      const latestVersion = versionsWithArtifactsAndScans.find(
        (v) => v.version === pkg.latestVersion,
      );
      const manifest = (latestVersion?.manifest ?? {}) as Record<string, unknown>;
      ...
      const transformSecurityScan = (
        ...
        const vulnFindings = ((scans.vulnerability as Record<string, unknown>)?.findings ??
          []) as Array<Record<string, unknown>>;
        const secretFindings = ((scans.secrets as Record<string, unknown>)?.findings ??
          []) as unknown[];
        const maliciousFindings = ((scans.malicious as Record<string, unknown>)?.findings ??
          []) as unknown[];
        const staticFindings = ((scans.static_analysis as Record<string, unknown>)?.findings ??
          []) as unknown[];
        ...
        const reportDomains = report?.domains as
        ...
        const findings = reportFindings
          ...
          .sort(
            (a, b) =>
              (SEVERITY_ORDER[a.severity as string] ?? 4) -
              (SEVERITY_ORDER[b.severity as string] ?? 4),
          )
          .map((f) => ({
            id: f.id as string,
        ...
        return {
          ...
          summary: {
            components:
              ((report?.sbom as Record<string, unknown>)?.component_count as number) ??
  ...
  fastify.post('/@:scope/:package/claim', {
    ...
    handler: async (request) => {
      ...
      if (!parsedRepo) {
        throw new BadRequestError(
          'Invalid GitHub repository format. Use format "owner/repo" or full GitHub URL',
        );
      }
      ...
      const verificationResult = await verifyPackageClaim(name, repoToVerify, githubUsername);
      ...
      // Claim the package
      const claimedPackage = await packageRepo.claimPackage(name, user.userId, repoToVerify);

      // Fetch GitHub stats asynchronously (non-blocking)
      fetchGitHubRepoStats(repoToVerify)
        .then((stats) => {
          if (stats) {
            packageRepo
              .updateGitHubStats(claimedPackage.id, stats)
              .catch((err: unknown) => fastify.log.error({ err }, 'Failed to update GitHub stats'));
          }
        })
        .catch((err: unknown) => fastify.log.error({ err }, 'Failed to fetch GitHub stats'));
