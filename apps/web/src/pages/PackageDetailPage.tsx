...
function detectPlatform(): { os: string; arch: string } {
  ...
  if (
    ...
    (os === 'darwin' && navigator.platform === 'MacIntel' && navigator.maxTouchPoints > 1)
  ) {
    arch = 'arm64';
  } else {
    arch = 'x64';
...
export default function PackageDetailPage() {
  ...
  const [selectedPlatform, setSelectedPlatform] = useState<{ os: string; arch: string }>({
    os: 'linux',
    arch: 'arm64',
  });
  ...
  return (
    <div className="min-h-screen flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6">
          <div className="flex flex-col sm:flex-row items-start gap-5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-mpak-gray-600 mb-3">
      ...
      {latestVersionData?.security_scan?.status === 'completed' &&
        latestVersionData.security_scan.certification?.level != null &&
        latestVersionData.security_scan.certification.level > 0 && (
          <div className="bg-surface-raised border-b border-white/[0.08]">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        {latestVersionData.security_scan.certification.controls_passed}/
                        {latestVersionData.security_scan.certification.controls_total} controls
                        passed
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_320px] gap-8 md:gap-12">
            ...
            <div className="min-w-0">
              ...
              {activeTab === 'tools' && pkg.tools && pkg.tools.length > 0 && (
                <section>
                  <h2 className="text-base font-semibold text-mpak-gray-900 mb-4">
                    Tools{' '}
                    <span className="text-mpak-gray-500 font-normal">({pkg.tools.length})</span>
                  </h2>
                  <div className="space-y-3">
                    {pkg.tools.map((tool) => (
                      <div
                        key={tool.name}
                        className="p-4 bg-surface-raised border border-white/[0.08] rounded-lg"
                      >
                        <div className="flex items-start gap-3">
                          ...
                          <div className="min-w-0">
                            <div className="font-mono text-sm font-semibold text-mpak-gray-900">
                              {tool.name}
                            </div>
                            {tool.description && (
                              <div className="text-sm text-mpak-gray-500 mt-1">
                                {tool.description}
                              </div>
                            )}
                          </div>
              ...
              {activeTab === 'install' && (
                <>
                  {/* Installation */}
                  <section className="mb-10">
                    <h2 className="text-base font-semibold text-mpak-gray-900 mb-3">
                      Installation
                    </h2>
                ...
                <>
                  {!latestVersionData?.security_scan && (
                    <div className="text-center py-12">
                      <svg
                        aria-hidden="true"
                        className="w-12 h-12 text-mpak-gray-400 mx-auto mb-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      <p className="text-mpak-gray-500">
                        Security scan not yet available for v{pkg.latest_version}
                      </p>
                    </div>
                  )}

                  {latestVersionData?.security_scan?.status === 'pending' ||
                  latestVersionData?.security_scan?.status === 'scanning' ? (
                    <div className="text-center py-12">
                      <svg
                        aria-hidden="true"
                      ...
                      <p className="text-accent-gold-400 font-medium">
                        Security scan in progress...
                      </p>
                      <p className="text-sm text-mpak-gray-500 mt-1">
                  ...
                  {latestVersionData?.security_scan?.status === 'failed' && (
                    <div className="text-center py-12">
                      <svg
                        aria-hidden="true"
                        className="w-12 h-12 text-terminal-error/60 mx-auto mb-4"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
                      ...
                      {pkg.versions.map((version) => (
                        ...
                        >
                          <div className="flex items-center justify-between mb-2">
                            <div className="flex items-center gap-2">
                              <span className="font-mono font-semibold text-mpak-gray-900">
                                v{version.version}
                              </span>
                              {version.prerelease && (
                                <span className="text-xs bg-accent-gold-400/15 text-accent-gold-400 px-2 py-0.5 rounded font-medium">
                                  pre-release
                                </span>
                              )}
                              {version.version === pkg.latest_version && (
                                <span className="text-xs bg-accent-gold-400 text-mpak-dark px-2 py-0.5 rounded font-medium">
                                  latest
                                </span>
                              )}
                            </div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {version.artifacts.map((artifact) => (
                                  <a
                                    ...
                                    href={api.getPackageDownloadUrl(pkg.name, version.version, {
                                      os: artifact.os,
                                      arch: artifact.arch,
                                    })}
            ...
            <aside className="md:sticky md:top-24 h-fit space-y-4">
              <div className="bg-surface-raised border border-white/[0.08] rounded-lg p-5">
                <h3 className="text-[11px] font-semibold text-mpak-gray-400 uppercase tracking-wide mb-4">
                  Version
                </h3>
                <select
                ...
                >
                  {pkg.versions?.map((v) => (
                    <option key={v.version} value={v.version}>
                      {v.version}
                      {v.version === pkg.latest_version ? ' (latest)' : ''}
                    </option>
                  ))}
                </select>
                ...
                {availablePlatforms.length > 0 && (
                  <div className="mb-4">
                    <h3 className="text-[11px] font-semibold text-mpak-gray-400 uppercase tracking-wide mb-3">
                      Platform
                    </h3>
                    <div className="flex flex-wrap gap-2">
                      {availablePlatforms.map((platform) => {
                        const isSelected =
                          selectedPlatform.os === platform.os &&
                          selectedPlatform.arch === platform.arch;
                        const isDetected =
                          detectedPlatform.os === platform.os &&
                          detectedPlatform.arch === platform.arch;
                        return (
                          <button
                            type="button"
                            key={`${platform.os}-${platform.arch}`}
                            onClick={() =>
                              setSelectedPlatform({ os: platform.os, arch: platform.arch })
                            }
                            className={`
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-mpak-gray-900 capitalize">
                      {pkg.server_type}
                    </span>
                  </div>
                  {pkg.license && (
                  ...
                  {selectedArtifact && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono font-medium text-mpak-gray-900">
              ...
              <div className="bg-surface-raised border border-white/[0.08] rounded-lg p-5">
                <a
                  href={api.getPackageDownloadUrl(pkg.name, pkg.latest_version, {
                    os: 'any',
                    arch: 'any',
                  })}
                ...
                {selectedArtifact && (
                  <p className="text-center text-xs text-mpak-gray-400 mt-2">
                    {selectedArtifact.os === 'any'
                      ? 'Universal bundle'
                      : `${selectedArtifact.os}/${selectedArtifact.arch}`}{' '}
                    · {formatBytes(selectedArtifact.size)}
                  </p>
                )}
              </div>
