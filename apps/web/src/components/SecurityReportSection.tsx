...
function getCertificationBadgeStyle(level: number | null): {
  bg: string;
  text: string;
  border: string;
} {
  switch (level) {
    case 1:
      return {
        bg: 'bg-terminal-info/15',
        text: 'text-terminal-info',
        border: 'border-terminal-info/25',
      };
    case 2:
      return {
        bg: 'bg-terminal-success/15',
        text: 'text-terminal-success',
        border: 'border-terminal-success/25',
      };
    case 3:
      return {
    ...
    case 4:
      return {
        bg: 'bg-accent-gold-400/15',
        text: 'text-accent-gold-400',
        border: 'border-accent-gold-border',
      };
    default:
...
function getRiskScoreIcon(score: string | null) {
  ...
  if (score === 'LOW') {
    return (
      <svg
        aria-hidden="true"
        className={`${iconClass} text-terminal-success`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
    ...
    return (
      <svg
        aria-hidden="true"
        className={`${iconClass} text-accent-gold-400`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
    ...
    return (
      <svg
        aria-hidden="true"
        className={`${iconClass} text-terminal-error`}
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
  ...
  return (
    <svg
      aria-hidden="true"
      className={`${iconClass} text-mpak-gray-400`}
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
    >
    ...
    return (
      <div className="bg-surface-raised rounded-lg border border-white/[0.08] p-4 mb-6">
        <div className="flex items-center gap-3">
          <svg
            aria-hidden="true"
            className="w-5 h-5 text-mpak-gray-400"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
          ...
          <span className="text-mpak-gray-500 text-sm">
            Security scan not yet available for v{version}
          </span>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-terminal-info/10 rounded-lg border border-terminal-info/20 p-4 mb-6">
        <div className="flex items-center gap-3">
          <svg
            aria-hidden="true"
            className="w-5 h-5 text-terminal-info animate-spin"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle
          ...
          <span className="text-terminal-info text-sm font-medium">
    ...
    return (
      <div className="bg-terminal-error/10 rounded-lg border border-terminal-error/20 p-4 mb-6">
        <div className="flex items-center gap-3">
          <svg
            aria-hidden="true"
            className="w-5 h-5 text-terminal-error"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
  ...
  return (
    <div className={`rounded-lg border p-4 mb-6 ${getRiskScoreColor(securityScan.risk_score)}`}>
      ...
      {certification && certification.level !== null && certification.level > 0 && (
        <div
          className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full ${certStyle.bg} ${certStyle.border} border mb-3`}
        >
          <svg
            aria-hidden="true"
            className={`w-4 h-4 ${certStyle.text}`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
            <path
        ...
        <div className="mt-4 pt-4 border-t border-white/[0.1] space-y-4">
              ...
              <div
                ...
                className={`p-3 rounded-lg border ${
                  result.success
                    ...
                    : 'bg-terminal-error/10 border-terminal-error/20'
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  ...
                  {result.success ? (
                    <svg
                      aria-hidden="true"
                      className="w-4 h-4 text-terminal-success"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
                    ...
                    <svg
                      aria-hidden="true"
                      className="w-4 h-4 text-terminal-error"
                      fill="currentColor"
                      viewBox="0 0 20 20"
                    >
                      <path
                ...
                <div className="text-xs text-mpak-gray-500">
                  ...
                  {result.error && <span className="text-terminal-error">{result.error}</span>}
                </div>
              </div>
            ))}
          </div>
          ...
          {securityScan.scans.sbom?.findings && securityScan.scans.sbom.findings.length > 0 && (
            <div>
              <h4 className="text-sm font-semibold text-mpak-gray-700 mb-2">
                Top Dependencies (
                {Math.min(10, securityScan.scans.sbom.findings.filter((f) => f.purl).length)} of{' '}
                {securityScan.scans.sbom.findings.filter((f) => f.purl).length})
              </h4>
