...
function buildComponents(accent: Accent): Components {
  ...
  return {
    ...
    p: ({ children }) => <p className="text-mpak-gray-600 mb-4 leading-relaxed">{children}</p>,
    a: ({ href, children }) => (
    ...
    img: ({ src, alt }) => <img src={src} alt={alt || ''} className="inline-block h-6 mr-1" />,
    table: ({ children }) => (
      <div className="overflow-x-auto mb-4">
        <table className="min-w-full border-collapse text-sm">{children}</table>
      </div>
    ),
    thead: ({ children }) => <thead className="border-b border-white/[0.12]">{children}</thead>,
    ...
    td: ({ children }) => <td className="px-3 py-2 text-mpak-gray-600">{children}</td>,
    pre: ({ children }) => (
...
export function MarkdownRenderer({ children, accent = 'gold', className }: MarkdownRendererProps) {
  return (
    <div className={className}>
      <ReactMarkdown remarkPlugins={[remarkGfm]} skipHtml components={buildComponents(accent)}>
        {children}
      </ReactMarkdown>
    </div>
  );
