...
export default function ScanTriggerButton({ packageName, version }: ScanTriggerButtonProps) {
  ...
  async function triggerScan() {
    ...
    try {
      const response = await httpClient.post<{
        ...
        scanId?: string;
        message: string;
      }>('/app/scan-trigger', { packageName, version });
      setResult({ success: response.data.success, message: response.data.message });
    } catch (err) {
      const axiosErr = err as { response?: { data?: { error?: string } }; message?: string };
      const message =
        axiosErr.response?.data?.error || axiosErr.message || 'Failed to trigger scan';
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {result && (
        <span
          className={`text-xs ${result.success ? 'text-terminal-success' : 'text-terminal-error'}`}
        >
          {result.message}
        </span>
      )}
