...
describe('handlePull', () => {
  ...
  it('calls downloadBundle with parsed name and version', async () => {
    ...
    expect(mockDownloadBundle).toHaveBeenCalledWith('@scope/test-bundle', '1.2.0', {
      os: 'darwin',
      arch: 'arm64',
    });
  });
  ...
  it('passes undefined version when none specified', async () => {
    ...
    expect(mockDownloadBundle).toHaveBeenCalledWith('@scope/test-bundle', undefined, {
      os: 'darwin',
      arch: 'arm64',
    });
  });
  ...
  it('writes to --output path when specified', async () => {
    ...
    expect(writeFileSync).toHaveBeenCalledWith('/tmp/my-bundle.mcpb', bundleData);
  });
