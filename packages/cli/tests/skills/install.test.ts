...
describe('handleSkillInstall', () => {
  ...
  it('creates skills directory and extracts with unzip', async () => {
    ...
    expect(writeFileSync).toHaveBeenCalledWith(expect.stringContaining('skill-'), skillData);
  ...
  it('cleans up temp file after extraction', async () => {
    ...
    expect(rmSync).toHaveBeenCalledWith(expect.stringContaining('skill-'), { force: true });
  });
  ...
  it('exits with error if already installed without --force', async () => {
    ...
    expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('already installed'));
    expect(stderrSpy).toHaveBeenCalledWith(expect.stringContaining('--force'));
  });
